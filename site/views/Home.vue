<template>
  <div id="app">
    <div class="main-header">
      ...
      <div class="main-header-title"><a href="/">IKEA-Docs</a></div>
      <ul class="main-header-nav">
        <li class="active">
          <a href="https://github.com/gxkai/gl" target="_blank">资源</a>
        </li>
      </ul>
    </div>
    <div class="main-">
      <div
        class="main-left"
        :style="{ display: showNav ? 'block' : 'none', height: leftHeight }"
      >
        <ul>
          <!--过滤index，首字母大写-->
...
export default {
  ...
  computed: {
    navigator() {
      return routerMds.sort(this._sortBy("name")).filter(item => {
...
#app .main-header .main-header-title a {
  font-size: 25px;
...
#app .fixed-bottom {
  ...
  height: 15px;
  cursor: pointer;
  border-top: 4px solid #41a259;
  border-bottom: 4px solid #41a259;
}
#app .fixed-bottom:after {
  content: "";
  display: block;
  width: 100%;
  height: 4px;
...
#app h2:hover .header-anchor,
#app h3:hover .header-anchor {
  opacity: 1;
}
#app .container-nav {
  position: fixed;
